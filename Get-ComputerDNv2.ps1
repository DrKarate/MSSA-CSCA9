...
<#
.Synopsis
Script gets the DN/ADSPath, IP Address and online state of a list of computers.  It accepts a list of NETBIOS names or FQDNs. Searches Forest root by default
...
#>
    ...
    $server
        ...
        if ($computer.Length -gt 1){
            ...
            if ($retval){$dnsHostName = ($retval.Properties).dnshostname}
            #If IP Address returned, use NBTName instead
            if([Net.IPAddress]::TryParse($dnshostname,[ref]$null)){$dnshostname = $NBTName}
            ...
            if ($DNShostname){$PingName= $DNShostname}Else{$PingName = $NBTName}
            ...
            if($IPAddress -eq $null){$IPAddress = "Not Found"}
        ...
        if (($DNSHostName).length -eq 0){$DNSHostName = $objPing.ProtocolAddressResolved.toString().ToUpper()}
        if (($DNSHostName).length -eq 0){$DNShostname = $NBTName}
...
{
    [CmdletBinding(DefaultParameterSetName="MyDomain")]
    Param
    (
    ...
    if($UseMyDomain) {$Server = $MyDomain.ToString()}
    ELSE{
        #The AD forest domain is used by default
        $SearchRoot = {Area52.Afnoapps.usaf.mil}

        #UseParentDomain: Search using a GC in parent domain
        if($UseParentDomain){$SearchRoot = Area52.Afnoapps.usaf.mil}

        Write-Host "Getting list of all GC Server(s) for $SearchRoot"
        $gc = get-addomaincontroller -server $SearchRoot
